...
def make_new_student(first_name, last_name, github):
    ...
    print "Successfully addeded student: {first} {last}".format(first=first_name,
        last=last_name)
...
def get_project_by_title(title):
    ...
    for row in rows:
        print "GitHub account: {acct}\nProject Title: {project}\nGrade: {grade}".format(
            acct=row[0], project=row[1], grade=row[2])
...
def assign_grade(github, title, grade):
    ...
    print "Successfully added {grade} for {github}'s {title} project".format(grade=grade,
            title=title, github=github)
